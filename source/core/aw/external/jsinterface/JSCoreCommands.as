	...
	internal class JSCoreCommands extends Object{
		...
		*/
		static internal const JSI_COMMANDS:Array = [
			...
			"JSIElement=function(){};JSIElement.EmbedNodeName='embed';JSIElement.get=function(id,url){var r;if(id){var d=JSI.d();r=JSI.isIE()? d[id]:JSIElement.getEmbedNode(d,id);if(!r){r=d.getElementById(id);};};if(!r){r=JSIElement.find(url);};return r;};",
			"JSIElement.getEmbedNode=function(d,id){var d=JSI.d();var r;if(id in window){r=JSIElement.getEmbed(window[id]);};if(!r && id in d){r=JSIElement.getEmbed(d[id]);};if(!r && 'all' in d && id in d.all){r=JSIElement.getEmbed(d.all[id]);};return r;};JSIElement.isEmbed=function(node){if(node instanceof HTMLElement && node.nodeName.toLowerCase()==JSIElement.EmbedNodeName){return true;}else{return false;};};",
